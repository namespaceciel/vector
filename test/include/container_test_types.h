...
struct TypeID {
  friend bool operator==(TypeID const& LHS, TypeID const& RHS) { return LHS.m_id == RHS.m_id; }
  friend bool operator!=(TypeID const& LHS, TypeID const& RHS) { return LHS.m_id != RHS.m_id; }
...
struct AllocatorConstructController {
  ...
  bool check(detail::TypeID const& tid) {
    ...
    if (m_expected_count == -1 || --m_expected_count == -1) m_expected_args = nullptr;
    return res;
  }

  // Return true iff there is an unchecked construction expression.
  bool unchecked() { return m_expected_args != nullptr; }
  ...
  bool isInAllocatorConstruct() const { return m_allow_constructions; }

  void inAllocatorConstruct(bool value = true) { m_allow_constructions = value; }

  void allowUnchecked(bool value = true) { m_allow_unchecked = value; }
...
struct ExpectConstructGuard {
  ...
  ~ExpectConstructGuard() { assert(!getConstructController()->unchecked()); }
};
...
class ContainerTestAllocator {
 ...
 public:
  ...
  ContainerTestAllocator() noexcept : controller(getConstructController()) {}

  explicit ContainerTestAllocator(ConstructController* c) : controller(c) {}

  template <class U>
  ContainerTestAllocator(ContainerTestAllocator<U, AllowConstructT> other) noexcept : controller(other.controller) {}
...
struct CopyInsertable {
  ...
  explicit CopyInsertable(int val) : data(val), copied_once(false), constructed_under_allocator(false) {
  ...
  CopyInsertable(CopyInsertable const& other) : data(other.data), copied_once(true), constructed_under_allocator(true) {
    assert(getConstructController()->isInAllocatorConstruct());
    assert(other.copied_once == false);
    other.copied_once = true;
  }

  CopyInsertable(CopyInsertable& other) : data(other.data), copied_once(true), constructed_under_allocator(true) {
    assert(getConstructController()->isInAllocatorConstruct());
    assert(other.copied_once == false);
    other.copied_once = true;
  }
  ...
  ~CopyInsertable() { assert(constructed_under_allocator == getConstructController()->isInAllocatorConstruct()); }
...
using vector = ciel::vector<T, ContainerTestAllocator<T, T>>;
template <class T = CopyInsertable<1>>

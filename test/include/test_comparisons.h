...
TEST_NODISCARD constexpr bool testComparisonsComplete(const T& t1, const U& t2, bool isEqual, bool isLess,
                                                      bool isGreater) {
...
template <class T, class U = T>
TEST_NODISCARD constexpr bool testComparisons(const T& t1, const U& t2, bool isEqual, bool isLess) {
...
TEST_NODISCARD constexpr bool testComparisonsValues(Param val1, Param val2) {
...
template <class T, class U = T>
TEST_NODISCARD constexpr bool testEquality(const T& t1, const U& t2, bool isEqual) {
  if (isEqual) {
    if (!(t1 == t2)) return false;
    if (!(t2 == t1)) return false;
    if ((t1 != t2)) return false;
...
//  Easy call when you can init from something already comparable.
template <class T, class Param>
TEST_NODISCARD constexpr bool testEqualityValues(Param val1, Param val2) {
  const bool isEqual = val1 == val2;
...
struct LessAndEqComp {
  ...
  friend constexpr bool operator<(const LessAndEqComp& lhs, const LessAndEqComp& rhs) { return lhs.value < rhs.value; }

  friend constexpr bool operator==(const LessAndEqComp& lhs, const LessAndEqComp& rhs) {
...
class ThreeWayComparable {
 public:
  ...
  friend constexpr std::strong_ordering operator<=>(const ThreeWayComparable&,
                                                    const ThreeWayComparable&) noexcept = default;

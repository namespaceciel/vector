...
// This unique behavior is particularly useful for testing the shrink_to_fit functionality in ciel::vector,
...
#if TEST_STD_VER >= 23
  constexpr_CXX23 std::allocation_result<T*> allocate_at_least(std::size_t n) {
    if (n < min_elements) n = min_elements;
    min_elements += 1000;
...
constexpr bool operator==(increasing_allocator<T>, increasing_allocator<U>) noexcept {
  return true;
}
...
class min_size_allocator {
 public:
  ...
  template <typename U>
  constexpr min_size_allocator(const min_size_allocator<MinAllocSize, U>&) noexcept {}

  TEST_NODISCARD constexpr T* allocate(std::size_t n) {
    if (n < MinAllocSize) n = MinAllocSize;
    return static_cast<T*>(::operator new(n * sizeof(T)));
  }

  constexpr void deallocate(T* p, std::size_t) noexcept { ::operator delete(static_cast<void*>(p)); }
...
constexpr bool operator==(const min_size_allocator<MinAllocSize, T>&, const min_size_allocator<MinAllocSize, U>&) {
  return true;
}
...
class pow2_allocator {
 public:
  ...
  template <typename U>
  constexpr pow2_allocator(const pow2_allocator<U>&) noexcept {}
 ...
 private:
  constexpr std::size_t next_power_of_two(std::size_t n) const {
    if ((n & (n - 1)) == 0) return n;
...
template <typename T, typename U>
constexpr bool operator==(const pow2_allocator<T>&, const pow2_allocator<U>&) {
  return true;
}

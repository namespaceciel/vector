...
struct AllocController {
 ...
 public:
  ...
  bool checkAlloc(void* p, std::size_t s) const { return p == last_alloc_pointer && s == last_alloc_size; }

  bool checkAllocAtLeast(void* p, std::size_t s, size_t a) const {
    return p == last_alloc_pointer && s <= last_alloc_size && a <= last_alloc_align;
  }

  bool checkAllocAtLeast(void* p, std::size_t s) const { return p == last_alloc_pointer && s <= last_alloc_size; }
  ...
  bool checkDealloc(void* p, std::size_t s) const { return p == last_dealloc_pointer && s == last_dealloc_size; }
  ...
  bool checkConstruct(Alloc const&, Tp* p) const {
    ...
    if (last_construct_pointer != p) return false;
    if (last_construct_alloc != expectAlloc) return false;
    if (last_construct_type != expectTp) return false;
    if (last_construct_args != expectArgs) return false;
    return true;
  }
  ...
  void countIsEqual() { ++is_equal_count; }

  bool checkIsEqualCalledEq(int n) const { return is_equal_count == n; }
...
class CountingAllocator {
 public:
  ...
  CountingAllocator(CountingAllocator const& other) : P(other.P) { P->copy_constructed += 1; }

  CountingAllocator(CountingAllocator&& other) : P(other.P) { P->move_constructed += 1; }

  template <class U>
  CountingAllocator(CountingAllocator<U, ID> const& other) noexcept : P(other.P) {
    P->copy_constructed += 1;
  }

  template <class U>
  CountingAllocator(CountingAllocator<U, ID>&& other) noexcept : P(other.P) {
 ...
 private:
  template <class Tp, std::size_t XID>
...
class MinAlignedAllocator {
 public:
  ...
  MinAlignedAllocator(MinAlignedAllocator const& other) : P(other.P) { P->copy_constructed += 1; }

  MinAlignedAllocator(MinAlignedAllocator&& other) : P(other.P) { P->move_constructed += 1; }

  template <class U>
  MinAlignedAllocator(MinAlignedAllocator<U> const& other) noexcept : P(other.P) {
    P->copy_constructed += 1;
  }

  template <class U>
  MinAlignedAllocator(MinAlignedAllocator<U>&& other) noexcept : P(other.P) {
    P->move_constructed += 1;
  }
 ...
 private:
  ...
  static bool is_max_aligned(void* p) { return reinterpret_cast<std::uintptr_t>(p) % BlockSize == 0; }
...
class NullAllocator {
 public:
  ...
  NullAllocator(NullAllocator const& other) : P(other.P) { P->copy_constructed += 1; }

  NullAllocator(NullAllocator&& other) : P(other.P) { P->move_constructed += 1; }

  template <class U>
  NullAllocator(NullAllocator<U> const& other) noexcept : P(other.P) {
    P->copy_constructed += 1;
  }

  template <class U>
  NullAllocator(NullAllocator<U>&& other) noexcept : P(other.P) {
    P->move_constructed += 1;
  }

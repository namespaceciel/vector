...
class DeathTest {
 public:
  ...
  void PrintFailureDetails(std::string_view invocation, std::string_view failure_description, std::string_view stmt,
                           DeathCause cause) const {
    std::fprintf(stderr, "Failure: %s( %s ) failed!\n(reason: %s)\n\n", invocation.data(), stmt.data(),
 ...
 private:
  ...
  [[noreturn]] void RunForChild(Func&& f) {
    ...
    auto DupFD = [](int DestFD, int TargetFD) {
      ...
      if (dup_result == -1) StopChildProcess(DeathCause::SetupFailure);
    };
...
bool ExpectDeath(const std::array<DeathCause, N>& expected_causes, const char* stmt, Func&& func,
  ...
  for (auto cause : expected_causes) assert(IsValidCause(cause));

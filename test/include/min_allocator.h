...
class safe_allocator {
 public:
  ...
  constexpr T* allocate(std::size_t n) {
    ...
    if (!TEST_IS_CONSTANT_EVALUATED) std::memset(static_cast<void*>(memory), 0, sizeof(T) * n);

    return memory;
  }

  constexpr void deallocate(T* p, std::size_t n) {
    if (!TEST_IS_CONSTANT_EVALUATED) DoNotOptimize(std::memset(static_cast<void*>(p), 0, sizeof(T) * n));

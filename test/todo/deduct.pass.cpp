...
constexpr bool tests() {
  //  Test the explicit deduction guides
  {
    ...
    ciel::vector vec(std::begin(arr), std::end(arr));

    static_assert(std::is_same_v<decltype(vec), ciel::vector<int>>, "");
    assert(std::equal(vec.begin(), vec.end(), std::begin(arr), std::end(arr)));
  }

  {
    const long arr[] = {INT_MAX, 1L, 2L, 3L};
    ciel::vector vec(std::begin(arr), std::end(arr), std::allocator<long>());
...

  {
    ...
    //  ciel::vector vec(std::allocator<int>()); // vector (allocator &)
  }

  {
    ciel::vector vec(1, A{});  // vector (size_type, T)
  ...
  {
    ciel::vector vec(1, A{}, test_allocator<A>());  // vector (size_type, T, allocator)
    static_assert(std::is_same_v<decltype(vec)::value_type, A>, "");
  ...
  {
    ciel::vector vec{1U, 2U, 3U, 4U, 5U};  // vector(initializer-list)
  ...
  {
    ciel::vector vec({1.0, 2.0, 3.0, 4.0}, test_allocator<double>());  // vector(initializer-list, allocator)
  ...
  {
    ciel::vector<long double> source;
    ciel::vector vec(source);  // vector(vector &)
  ...
  {
    {
      ciel::vector c(std::from_range, std::array<int, 0>());
      static_assert(std::is_same_v<decltype(c), ciel::vector<int>>);
    }

    {
      using Alloc = test_allocator<int>;
      ciel::vector c(std::from_range, std::array<int, 0>(), Alloc());
      static_assert(std::is_same_v<decltype(c), ciel::vector<int, Alloc>>);
    }
  }
#endif
  ...
  {
    ciel::vector vec(3, true);  // vector(initializer-list)
...

  {
    ...
    {
      ciel::vector<short, Alloc> source;
      ciel::vector vec(source, Alloc(2));
      static_assert(std::is_same_v<decltype(vec), decltype(source)>);
    }

    {
      ciel::vector<short, Alloc> source;
      ciel::vector vec(source, ConvertibleToAlloc(2));
      static_assert(std::is_same_v<decltype(vec), decltype(source)>);
    }

    {
      ciel::vector<short, Alloc> source;
      ciel::vector vec(std::move(source), Alloc(2));
      static_assert(std::is_same_v<decltype(vec), decltype(source)>);
    }

    {
      ciel::vector<short, Alloc> source;
      ciel::vector vec(std::move(source), ConvertibleToAlloc(2));
      static_assert(std::is_same_v<decltype(vec), decltype(source)>);
    }
  }

  SequenceContainerDeductionGuidesSfinaeAway<ciel::vector, ciel::vector<int>>();

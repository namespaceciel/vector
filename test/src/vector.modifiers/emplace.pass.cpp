...
constexpr void test() {
  ...
  // Emplace at the end of a vector with increasing size
  {
    ...
    for (std::size_t n = 3; n != 100; ++n) {
      ...
      assert(it == v.end() - 1);
      assert(v.size() == n + 1);
      for (std::size_t i = 0; i != n + 1; ++i) assert(v[i] == T(i));
    }
  }
  ...
  // Emplace at the start of a vector with increasing size
  {
    ...
    for (std::size_t n = 3; n != 100; ++n) {
      ...
      assert(it == v.begin());
      assert(v.size() == n + 1);
      for (std::size_t i = 0; i != n + 1; ++i) assert(v[i] == T(n - i));
    }
  }
  ...
  // must make sure that it doesn't end up inserting from an element whose position has changed.
  {
    // When capacity must increase
    {
      ...
      assert(v[0] == T(2));
      assert(v[1] == T(1));
      if (has_moved_from_sentinel_value<T>::value) assert(v[2] == T(0));
    }
...
#if defined(_LIBCPP_VERSION) && !defined(TEST_HAS_NO_EXCEPTIONS)
  {
    ...
    ciel::vector<ThrowingMoveOnly, Allocator<ThrowingMoveOnly> > v;

...
constexpr void tests() {
  {
    T arr[] = {T(1), T(2), T(3)};
    using Vector = ciel::vector<T, Allocator<T> >;
  ...
  {
    using InnerVector = ciel::vector<T, Allocator<T> >;
    using Vector = ciel::vector<InnerVector, Allocator<InnerVector> >;
  ...
  // Make sure vector::erase works with move-only types
  {
    // When non-trivial
    {
      ciel::vector<MoveOnly, Allocator<MoveOnly> > v;
      v.emplace_back(1);
      v.emplace_back(2);
      v.emplace_back(3);
    ...
    // When trivial
    {
      ciel::vector<TrivialMoveOnly, Allocator<TrivialMoveOnly> > v;
      v.emplace_back(1);
      v.emplace_back(2);
      v.emplace_back(3);
      v.erase(v.begin(), v.begin() + 2);
...
int main(int, char**) {
  ...
  {
    Throws arr[] = {1, 2, 3};
    ciel::vector<Throws> v(arr, arr + 3);
    Throws::sThrows = true;
    v.erase(v.begin(), v.end() - 1);  // TODO: Ciel v.erase(v.begin(), --v.end());
    assert(v.size() == 1);
  ...
  // Real world example with std::string, mostly intended to test trivial relocation
  {
    ...
    ciel::vector<std::string> original = v;
  ...
  {
    Tracker tracker;
    ciel::vector<TrackedAssignment> v;

...
int main(int, char**) {
  using AllocVec = ciel::vector<int, throwing_allocator<int> >;
  try {  // vector()
    AllocVec vec;
  } catch (int) {
  }
  ...
  try {  // Throw in vector(size_type) from type
    ciel::vector<throwing_t> get_alloc(1);
  } catch (int) {
  }
  check_new_delete_called();
  ...
  try {  // Throw in vector(size_type, value_type) from type
    ...
    ciel::vector<throwing_t> get_alloc(1, v);
  } catch (int) {
  }
  check_new_delete_called();
  ...
  try {  // Throw in vector(size_type, const allocator_type&) from the type
    ciel::vector<throwing_t> vec(1, std::allocator<throwing_t>());
  } catch (int) {
  }
  check_new_delete_called();
  ...
  try {  // Throw in vector(size_type, value_type, const allocator_type&) from the type
    int throw_after = 1;
    throwing_t v(throw_after);
    ciel::vector<throwing_t> vec(1, v, std::allocator<throwing_t>());
  } catch (int) {
  }
  check_new_delete_called();
  ...
  try {  // Throw in vector(const vector&, const allocator_type&) from type
    ...
    int throw_after = 1;
    vec.emplace_back(throw_after);
    ciel::vector<throwing_t> vec2(vec, std::allocator<int>());
  } catch (int) {
  }
  check_new_delete_called();

  try {  // Throw in vector(vector&&, const allocator_type&) from type during element-wise move
    ciel::vector<throwing_t, test_allocator<throwing_t> > vec(test_allocator<throwing_t>(1));
    int throw_after = 10;
    throwing_t v(throw_after);
    vec.insert(vec.end(), 6, v);
    ciel::vector<throwing_t, test_allocator<throwing_t> > vec2(std::move(vec), test_allocator<throwing_t>(2));
  } catch (int) {
  }
  check_new_delete_called();
  ...
  try {  // Throw in vector(initializer_list<value_type>) from type
    int throw_after = 1;
    ciel::vector<throwing_t> vec({throwing_t(throw_after)});
  } catch (int) {
  }
  check_new_delete_called();
  ...
  try {  // Throw in vector(initializer_list<value_type>, const allocator_type&) constructor from type
    int throw_after = 1;
    ciel::vector<throwing_t> vec({throwing_t(throw_after)}, std::allocator<throwing_t>());
  } catch (int) {
  }
  check_new_delete_called();
